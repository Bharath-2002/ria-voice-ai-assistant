...
**Memory / state:** in‑call context (occasion, recipient, metal, budget, recommended products) lives in **Redis** keyed by the ElevenLabs conversation id; the catalogue results of the current search are cached there so `send_to_whatsapp` can page through them. Conversation transcripts/summaries are owned by **ElevenLabs** (the eval dashboard reads them on demand — we don't mirror calls). **Postgres** holds only evaluation results.
